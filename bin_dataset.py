...
class GoProSingleImageEventDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        split_path = os.path.join(self.dataroot, split_dir)

        # 检查目录是否存在
        if not os.path.exists(split_path):
            raise FileNotFoundError(f"Data split directory not found: {split_path}")

        # 获取所有文件索引（假设三个文件夹中的文件数量和命名一致）
        blur_folder = os.path.join(split_path, 'aps_raw_int10')
        gt_folder = os.path.join(split_path, 'gt_raw_int10')
        event_folder = os.path.join(split_path, 'events')
